...
Recognizing that LAD structures span broad, continuous megabase-scale genomic blocks rather than narrow, localized peaks, this pipeline bypasses standard peak-calling variants:

* **Tiling Window Construction:** Calculates the exact biological midpoint of every annotated gene body in the mouse genome. From each midpoint, it expands a standard 100kb genomic window (50kb upstream and 50kb downstream) using a safe boundary constraint to prevent chromosome coordinates from dropping below 1: Window Start = max(1, Gene Center - 50,000) and Window End = Gene Center + 50,000.
* **Iterative Matrix Generation:** Loops through compressed sample BED tracks (.bed.gz) and measures raw read overlaps within the 100kb boundaries via the countOverlaps() function.
* **Counts Per Million (CPM) Scaling:** Normalizes raw matrices using calculated sequencing depths to permit comparative mathematical operations. The normalized value is computed as: CPM = (Raw Counts / Total Read Depth) * 1,000,000.
* **Stringent Selection Metrics:** Extracts mutation-specific candidates by demanding a multi-tier threshold check:
    1. **Global Baseline Filter:** Average global CPM must be greater than or equal to 3 to eliminate low-mappability regions.
    2. **Enrichment Magnitude Check:** Group Raw count average must be greater than or equal to 150.
    3. **Directional Fold Change Constraint:** Captures true R179C enrichment at log2 fold change > 0.585, and true WT enrichment at log2 fold change < -0.585.
